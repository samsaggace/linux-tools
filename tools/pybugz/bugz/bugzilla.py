...
class Bugz:
	...
	def auth(self):
		...
		# check if auth file is present
		auth_file = os.path.join(os.environ['HOME'], AUTH_FILE)
		try:
			fd_auth_file = os.open(auth_file, os.O_RDONLY)
		except OSError:
			self.log('Auth file not found')
		else:
			auth_string = os.read(fd_auth_file, 1024)
			os.close(fd_auth_file)
		...
		try:
			...
			if hasattr(e, 'code'):
				if e.code != 401:
					raise RuntimeError("Failed to login (Error %d)" % e.code)
				else:
					self.log('Http authentification required')
					# prompt for username if we were not supplied with it
					if not self.httpuser:
						self.log('No http username given.')
						self.httpuser = self.get_input('Username: ')

					# prompt for password if we were not supplied with it
					if not self.httppassword:
						self.log('No http password given.')
						self.httppassword = getpass.getpass()

					if self.httpuser and self.httppassword:
						base64string = base64.encodestring('%s:%s' % (self.httpuser, self.httppassword))[:-1]
						req.add_header("Authorization", "Basic %s" % base64string)

						# save authentification string to auth file
						auth_file = os.path.join(os.environ['HOME'], AUTH_FILE)
						try:
							fd_auth_file = os.open(auth_file, os.O_WRONLY | os.O_CREAT)
						except OSError:
							self.log('Cannot create auth file not found')
						else:
							os.write(fd_auth_file, base64string)
							os.close(fd_auth_file)
							self.auth_string = base64string
		...
		if self.httpuser and self.httppassword:
			base64string = base64.encodestring('%s:%s' % (self.httpuser, self.httppassword))[:-1]
			req.add_header("Authorization", "Basic %s" % base64string)
		elif self.auth_string:
			req.add_header("Authorization", "Basic %s" % self.auth_string)
	...
	def search(self, query, comments = False, order = 'number',
		...
		if status == None or [s.upper() for s in status] == ['ALL']:
	...
	def get(self, bugid):
		...
		if bug and bug.attrib.has_key('error'):
			self.log("Bug %s not found !" % bugid)
			return None
		else:
			return etree
...
# Patch to be compatible with forge 44
...
# End patch
		...
		# Patch to convert utf-8 character 
		convparams = config.convtab['utftab'].copy()
		for k in convparams:
			for l in qparams:
				qparams[l] = qparams[l].replace(k, convparams[k])
		req_params = urlencode(qparams, True).replace('%25','%')
		# End patch
		req_url = urljoin(self.base, config.urls['modify'])
#		self.log("req_url : %s\nreq_params : %s\nconfig.headers : %s\n" % (req_url, req_params, config.headers))
		...
		resp_str = resp.read()
		try:
			re_bug = re.compile(r'<title>Bogue [0-9]+ trait.*</title>')
			bug_match = re_bug.search(resp_str) 
			if bug_match:
				return modified
			else:
				re_redmsg = re.compile(r'<td bgcolor="#ff0000">.*</td>', re.DOTALL)
				redmsg = re_redmsg.search(resp_str).group(0)
				re_brack = re.compile(r'<.*?>')
				re_newline = re.compile(r'\n')
				re_multispace = re.compile(r' +')
				redmsg = re_brack.sub(r'', redmsg)
				redmsg = re_newline.sub(r'', redmsg)
				redmsg = re_multispace.sub(r' ', redmsg)
				self.log("--------------- Error Message ---------------")
				self.log("%s" % redmsg)
				self.log("---------------------------------------------")

		except:
			self.log("Failed to get status from Bugzilla")
			return []
	...
	def post(self, product, component, title, description, url = '', assigned_to = '', cc = '', keywords = '', version = '', dependson = '', blocked = '', priority = '', severity = '', client = '', ref_produit = '', pays = '', sys_expl = '', site_origine = '', site_dest = ''):
		"""Post a bug
		...
		qparams['keywords'] = keywords
		qparams['cf_client'] = client
		qparams['cf_reference'] = ref_produit
		qparams['cf_pays'] = pays
		qparams['cf_sysexp'] = sys_expl
		qparams['cf_site_orig'] = site_origine
		qparams['cf_site_dest'] = site_dest
...
#		self.log("req_url : %s\nreq_params : %s\nconfig.headers : %s\n" % (req_url, req_params, config.headers))
		...
		try:
			re_bug = re.compile(r'<title>.*Bogue ([0-9]+) soumis</title>')
			bug_match = re_bug.search(resp_str)
			if bug_match:
