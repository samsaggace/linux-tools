...
class PrettyBugz(Bugz):
	options = {
		'base': make_option('-b', '--base', type='string',
							default = os.environ['URL_BUGZILLA'],
	...
	def search(self, *args, **kwds):
		...
		hide_status = kwds['hide_status']
		del kwds['hide_status']
		...
		self.listbugs(result, show_url, hide_status)
	...
	search.options = {
		...
		'hide_status': make_option('--hide-status', help='hide status of bugs',
								action = 'store_true', default = False),
		'show_url': make_option('--show-url', help='Show bug id as a url.',
								action = 'store_true', default = False),
	}

	def namedcmd(self, command, hide_status=False, show_url=False):
	...
	def post(self, product = None, component = None,
			...
			client = None, product_ref = None, country = None, os = None, 
			orig_site = None, dest_site = None):
		"""Post a new bug"""
		...
		if not batch:
			...
			if prodversion is None:
				prodversion = self.get_input('Enter version (default: 0.0): ')
			else:
				self.log('Enter version: %s' % prodversion)

			# check for component
			if component is None:
				component = self.get_input('Enter component (default: Sans objet): ')
			else:
				self.log('Enter component: %s' % component)

			# check for default priority
			if priority is None:
				priority_msg ='Enter priority (default: Normal): '
			...
			if severity is None:
				severity_msg ='Enter severity (default: Mineure): '
			...
			if not assigned_to:
				while not assigned_to or len(assigned_to) < 1:
					assigned_to = self.get_input('Enter assignee (eg. liquidx@gentoo.org): ')
			else:
				self.log('Enter assignee (eg. liquidx@gentoo.org): %s' % assigned_to)
			...
			# check for client
			if not client:
				while not client or len(client) < 1:
					client = self.get_input('Enter client: ')
			else:
				self.log('Enter client: %s' % client)

			# check for product reference
			if not product_ref:
				while not product_ref or len(product_ref) < 1:
					product_ref = self.get_input('Enter product_ref: ')
			else:
				self.log('Enter product_ref: %s' % product_ref)

			# check for country
			if country is None:
				country = self.get_input('Enter country (default: FR) : ')
			else:
				self.log('Enter country : %s' % country)	

			# check for os
			if os is None:
				os = self.get_input('Enter OS (default: Linux) : ')
			else:
				self.log('Enter OS : %s' % os)

			# check for site origine
			if orig_site is None:
				orig_site = self.get_input('Enter origine site (default: RMM) : ')
			else:
				self.log('Enter origine site : %s' % orig_site)

			# check for site destination
			if dest_site is None:
				dest_site = self.get_input('Enter destination site (default: RMM) : ')
			else:
				self.log('Enter destination site : %s' % dest_site)
		...
		if prodversion is None or prodversion == '' :
			prodversion = '0.0'
		...
		if product_ref is None or product_ref == '' :
			product_ref = 'TO-BE-DEFINED'
		if country is None or country == '' :
			country = 'FR'
		if os is None or os == '' :
			os = 'Linux'
		if orig_site is None or orig_site == '' :
			orig_site = 'RMM'
		if dest_site is None or dest_site == '' :
			dest_site = 'RMM'

		# raise an exception if mandatory fields are not specified.
		if assigned_to is None:
			raise RuntimeError('assigned_to  not specified')
		if product is None:
		...
		if client is None:
			raise RuntimeError('client not specified')
		if product_ref is None:
			raise RuntimeError('product_ref not specified')
		if country is None:
			raise RuntimeError('country not specified')
		if os is None:
			raise RuntimeError('os not specified')
		if orig_site is None:
			raise RuntimeError('orig_site not specified')
		if dest_site is None:
			raise RuntimeError('dest_site not specified')
		...
		print 'URL               : ' + url
		print 'Client            : ' + client
		print 'Product reference : ' + product_ref
		print 'Country           : ' + country
		print 'Operating system  : ' + os
		print 'Origine site      : ' + orig_site
		print 'Destination site  : ' + dest_site
		print 'Title             : ' + title
		...
		result = Bugz.post(self, product, component, title, description, url, assigned_to, cc, keywords, prodversion, dependson, blocked, priority, severity, client, product_ref, country, os, orig_site, dest_site)
	...
	post.options = {
		...
		'priority': make_option('--priority', choices=config.choices['priority'].values()),
		'severity': make_option('-S', '--severity', choices=config.choices['severity']),
		'client': make_option('--client', help = 'Client'), 
		'product_ref': make_option('--product-ref', help = 'Product reference'),
		'country': make_option('--country', help = 'Country'),
		'os': make_option('--os', help = 'Operating system'),
		'orig_site': make_option('--orig-site', help = 'Origine site'),
		'dest_site': make_option('--dest-site',	help = 'Destination site'),
	}
	...
	def listbugs(self, buglist, show_url=False, hide_status=False):
		for row in buglist:
			bugid = row['bugid']
			if show_url:
			...
			if row.has_key('assignee'): # Novell does not have 'assignee' field
				...
				line = '%s %-22s' % (line, assignee)
	
			# PATCH ACA@SAGEMcOM
			line = '%s %s' % (line, desc)
			#line = '%s %-28s %s' % (line, row['product'], desc)
