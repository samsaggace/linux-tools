...
"    endif
  ...
  " PATCH ACA@SAGEMCOM
  " Force signature if preferred since information is not kept through deciphering
	if (match(b:GPGOptions, "sign") <= 0)
   	if (exists("g:GPGPreferSign") && g:GPGPreferSign == 1)
      	let b:GPGOptions += ["sign"]
    	endif
   endif
...
function s:GPGEditOptions()
  ...
  if (match(bufname("%"), "^\\(GPGRecipients_\\|GPGOptions_\\)") != 0 && match(bufname("%"), "\.\\(gpg\\|asc\\|pgp\\)$") >= 0)
"  if (match(bufname("%"), "^\\(GPGRecipients_\\|GPGOptions_\\)") != 0)
