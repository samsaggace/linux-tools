...
sub filetypes {
    ...
    return ('make',TEXT)        if ($lc_basename =~ m/makefile/ 
                                   || $lc_basename eq 'gnumakefile'
							       || $lc_basename =~ m/\.ma*k$/ 
                                   || $lc_basename =~ m/\.inc$/ );
    #return ('make',TEXT)        if $lc_basename eq 'makefile';
    return ('rake','ruby',TEXT) if $lc_basename eq 'rakefile';
    return ('lkc',TEXT) if ($lc_basename eq 'kbuild' 
                           || $lc_basename eq 'kconfig' );
    return ('vim',TEXT) if $lc_basename =~ m/.*vimrc.*/;
